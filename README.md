...
- [POST APIs Detailed Contracts](#post-apis-detailed-contracts)
...
## POST APIs Detailed Contracts

This section documents the two primary POST APIs you asked for:
...
Create a new employee profile and upload a face image.
...
|---|---|---|---|---|
| `fullName` | string | Yes | Max length 255 | `Mahmoud Ahmed` |
| `email` | string | Yes | Must be a valid email, max length 255, unique | `mahmoud@company.com` |
| `phone` | string | Yes | Max length 30 | `+201001112223` |
| `department` | string | Yes | Max length 100 | `Engineering` |
| `position` | string | Yes | Max length 100 | `Software Engineer` |
| `joinDate` | string | Yes | Must be `YYYY-MM-DD` | `2026-03-16` |
| `faceImage` | file | Yes | Image file is required | `profile.jpg` |
...
curl -X POST "https://localhost:5001/api/employees" \
  ...
  -F "fullName=Mahmoud Ahmed" \
  -F "email=mahmoud@company.com" \
  -F "phone=+201001112223" \
  -F "department=Engineering" \
  -F "position=Software Engineer" \
  -F "joinDate=2026-03-16" \
  -F "faceImage=@C:/images/profile.jpg"
```
...
```json
{
  "uuid": "2f9a9be0-3df2-4f31-9efe-e345f6f8b2a9",
  ...
  "joinDate": "2026-03-16",
  "createdAt": "2026-03-16T09:43:21.1184423Z"
}
```
...
Or first model-validation error:
...
Mark attendance for a registered employee UUID.
...
| Field | Type | Required | Rules | Example |
|---|---|---|---|---|
| `uuid` | string | Yes | Must be a valid GUID string | `2f9a9be0-3df2-4f31-9efe-e345f6f8b2a9` |
| `latitude` | number | Yes | Must be between -90 and 90 | `30.0444` |
| `longitude` | number | Yes | Must be between -180 and 180 | `31.2357` |
| `timestamp` | string | No | Not sent by frontend. Server uses API hit time for attendance mark. | *(auto-generated by server)* |
...
```json
{
  "attendanceId": "att_dce2f3f5-5b35-48b2-9cc8-a8f2a6fdb923",
  "uuid": "2f9a9be0-3df2-4f31-9efe-e345f6f8b2a9",
  "markedAt": "2026-03-16T07:20:00.0000000Z",
  "status": "present"
}
```
...
- Active office location must be configured.
- Check-in must be inside a 100-meter office radius.
- Time must be within the allowed window (09:00 AM to 05:00 PM).
...
```json
{ "message": "Check-in is allowed only between 09:00 AM and 05:00 PM." }
```
...
```json
{ "message": "Check-in location is outside the allowed 100 meter office proximity. Distance: 250.21 meters, allowed: 100 meters." }
```

**404 Not Found**

```json
{ "message": "No employee found for this UUID." }
```
